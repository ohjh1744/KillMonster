...
public class ZoomState : AttackState
{
    ...
    void ZoomInAim()
    {
        _playerData.IsZoom = Zoom.줌인;
        _playerData.Aims[(int)Zoom.줌아웃].SetActive(false);
        ...
        _playerData.Aims[(int)Zoom.줌인].SetActive(true);
    ...
    void ZoomOutAim()
    {
        _playerData.IsZoom = Zoom.줌아웃;
        _playerData.Aims[(int)Zoom.줌인].SetActive(false);
        _anim.SetBool("isZoom", false);
        IZoomable zoomable = _playerData.FireWeapons[(int)_playerData.CurFireWeapon].GetComponent<IZoomable>();
        zoomable.ZoomOut(_virtualCamera);
        _playerData.Aims[(int)Zoom.줌아웃].SetActive(true);
    }
